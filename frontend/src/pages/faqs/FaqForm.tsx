...
export default function FaqForm() {
  ...
  const { data: faq, isLoading: loadingFaq } = useFaq(faqId);
  ...
  const handleSubmit = async (e?: React.SyntheticEvent) => {
    ...
    try {
      ...
      if (mode === 'edit' && id) {
        result = await updateFaq(Number(id), payload);
      } else {
  ...
  return (
    <>
      ...
      <div className="bg-bg-secondary border border-border-color rounded-2xl overflow-hidden shadow-[0_4px_12px_rgba(0,0,0,0.4)]">
        <form id="faq-form" onSubmit={handleSubmit} className="p-8 space-y-6">
          <div>
      ...
      <AlertModal
        ...
        onClose={() => {
          setAlertMsg('');
          if (shouldNavigate) navigate('/faqs');
        }}
      />
    </>
  );
